...
class FileSystemInfo {
	constructor(fs) {
		...
		this._fileTimestampQueue = new AsyncQueue({
			name: "file timestamp",
			parallelism: 30,
			processor: this._readFileTimestamp.bind(this)
		});
		this._contextTimestampQueue = new AsyncQueue({
	 ...
	 * @returns {boolean} true, if there is already info about this file
	 */
	hasFileTimestamp(path) {
		return this._fileTimestamps.has(path);
	}
	...
	getFileTimestamp(path, callback) {
		...
		this._fileTimestampQueue.add(path, callback);
	}

	/**
	 * @param {string} path file path
	 * @returns {boolean} true, if there is already info about this context
	 */
	hasContextTimestamp(path) {
		return this._contextTimestamps.has(path);
	}
	...
	getContextTimestamp(path, callback) {
		...
		if (cache !== undefined) return cache;
		this._contextTimestampQueue.add(path, callback);
	}
